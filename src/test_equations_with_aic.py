...
df["jaeb_predicted_basals"] = df.apply(
    lambda x: jaeb_basal_equation(x[tdd_key], x[carb_key]), axis=1
)
df["traditional_predicted_basals"] = df.apply(
    lambda x: traditional_basal_equation(x[tdd_key]), axis=1
)
...
jaeb_basal_aic = aic(2, jaeb_basal_sum_squared_errors)
traditional_basal_aic = aic(1, traditional_basal_sum_squared_errors)
...
df["jaeb_predicted_isf"] = df.apply(
    lambda x: jaeb_isf_equation(x[tdd_key], x[bmi_key]), axis=1
)
df["traditional_predicted_isf"] = df.apply(
    lambda x: traditional_isf_equation(x[tdd_key]), axis=1
)
...
df["jaeb_predicted_icr"] = df.apply(
    lambda x: jaeb_icr_equation(x[tdd_key], x[carb_key]), axis=1
)
df["traditional_predicted_icr"] = df.apply(
    lambda x: traditional_icr_equation(x[tdd_key]), axis=1
)
