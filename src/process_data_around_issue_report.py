...
def get_args():
    ...
    parser.add_argument(
        "-individual_report_results_path",
        dest="individual_report_results_path",
...
def get_sample_data_with_buffer(single_report, data, local_timezone):
    ...
    return buffered_sample_data, sample_start_time, sample_end_time
...
def check_settings_units(single_report):
    ...
    if (single_report["suspend_threshold"] < 0) | (single_report["suspend_threshold"] < 0) > 400:
...
def process_schedules(single_report):
    ...
    if pd.notnull(isf_schedule_string):
        isf_schedule_count, isf_median, isf_geomean = process_isf_schedule(isf_schedule_string)
...
def create_daily_5min_ts(buffered_sample_data):
...
def process_daily_data(single_report, buffered_sample_data, sample_start_time, local_timezone):
    daily_5min_ts = create_daily_5min_ts(buffered_sample_data)
    single_report, daily_5min_ts = process_basal_data(single_report, buffered_sample_data, daily_5min_ts)
    single_report, daily_5min_ts = process_bolus_data(single_report, buffered_sample_data, daily_5min_ts)
    single_report, daily_5min_ts = process_carb_data(single_report, buffered_sample_data, daily_5min_ts)
    daily_5min_ts["total_insulin_delivered"] = daily_5min_ts["basal_pulse_delivered"] + daily_5min_ts["normal"]

    daily_5min_ts = calculate_iob_for_timeseries(daily_5min_ts)

    local_start_time = sample_start_time.tz_convert(local_timezone)
    final_14day_ts = daily_5min_ts[daily_5min_ts["rounded_local_time"] >= local_start_time].copy()
    final_14day_ts["date"] = final_14day_ts["rounded_local_time"].dt.date

    basal_daily_amounts = final_14day_ts.groupby("date")["basal_pulse_delivered"].sum().values
    ...
    bolus_daily_amounts = final_14day_ts.groupby("date")["normal"].sum().values
    ...
    insulin_daily_amounts = final_14day_ts.groupby("date")["total_insulin_delivered"].sum().values
    ...
    carbs_daily_amounts = final_14day_ts.groupby("date")["carbs"].sum().values
...
def main(loop_id, dataset_path, individual_report_results_path, issue_reports):
    ...
    for report_idx in range(len(reports_from_one_id)):
        try:
            ...
            buffered_sample_data, sample_start_time, sample_end_time = get_sample_data_with_buffer(
                single_report, data, local_timezone
            )

            if len(buffered_sample_data) > 0:
                single_report["surrounding_data_available"] = True
                single_report = process_schedules(single_report)
                single_report = check_settings_units(single_report)
                single_report = process_cgm_data(single_report, buffered_sample_data, sample_start_time)
                single_report = process_daily_data(
            ...
            save_filename = "PHI-" + loop_id + "-summary-statistics-" + single_report["file_name"] + ".csv"
            save_path = os.path.join(individual_report_results_path, save_filename)
            single_report = pd.DataFrame(single_report).T
            single_report.to_csv(save_path, index=False)

        except Exception as e:
            print("{} -- {} FAILED with error: ".format(loop_id, single_report["file_name"]))
...
if __name__ == "__main__":
    ...
    args = get_args()
    loop_id, dataset_path, individual_report_results_path = (
        args.loop_id,
        args.dataset_path,
